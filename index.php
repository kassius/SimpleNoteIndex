...
/**
 *
 *  If you need to edit the HTML generated for each part you can edit here. 
 *
 */

class SNIMakeContent
{
	/**
	 *
	 *  Edit the return of this method to change the HTML generated by index page (list of files)
	 *  Keep the EOT's: they mean the begin and end of the string.
	 *
	 */

	static function index($items)
	{
		return <<<EOT
<div id="files">
<div id="files-header">.md Files</div>
{$items}
</div>
EOT;
	}
	...
	/**
	 *
	 *  Change the return of this method to effect the HTML for the index list items (*each* file name and link)
	 *
	 */
	...
	/**
	 * 
	 *  Change the return of this method to change HTML for the opened file markdown parsed / textarea.
	 *
	 */

	static function fileload($plain, $markdown, $fileuri)
	{
		return <<<EOT
<div id="text">
{$markdown}
...
<div id="edit">
<form method="post">
<textarea id="edittextarea" name="content">
{$plain}
</textarea>
<input type="hidden" name="fileuri" value="{$fileuri}" />
<input id="editbutton" name="save" type="submit" value="Save" />
</form>
</div>
...
EOT;
	...
	static function title($filename)
	{
		return "{$filename}";
	}
...
class SimpleNoteIndex
{
	...
	public function __construct($get, $post)
	{
		...
		$this->directory = "<a href=\"{$_SERVER['PHP_SELF']}\">".array_pop($stairway)."</a>";
	...
	public function index()
	{
		...
		for($i=0; $dir[$i]; $i++)
		{
			if(preg_match($this->regex, trim($dir[$i])) !== 0)
			{
				$file[$fc]['name'] = trim($dir[$i]);
				$file[$fc]['uri'] = urlencode(trim($dir[$i]));
				$fstr .= SNIMakeContent::indexitem($file[$fc]['uri'], $file[$fc]['name']);
			}
		}
	...
	{
		$filename = urldecode($fileuri);
		if(file_exists($filename))
		{
			$file = file_get_contents($filename);
			$this->directory .= " &middot; {$filename}";
			$this->title = SNIMakeContent::title($filename);
		}
		else
		{
